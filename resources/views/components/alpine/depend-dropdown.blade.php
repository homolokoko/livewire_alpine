@props(['model','list', 'placeholder'])
<div x-data="{
    selectOpen: false,
    selectedItem: @entangle($model),
    selectableItems: @entangle($list),
    ...
    selectItemsFindBestMatch(){
        ...
        for (var i = 0; i < this.selectableItems.length; i++) {
            var text = this.selectableItems[i].text.toLowerCase();
...
<button  x-ref="selectButton" @click="selectOpen=!selectOpen"
    ...
    <span x-text="selectedItem ? selectedItem.text : {{$placeholder}}" class="truncate">Select Item</span>
...
<ul x-show="selectOpen"
    ...
    <template x-for="item in selectableItems" :key="item.value">
        <li
            ...
            <span class="block font-medium truncate" x-text="item.text"></span>
        </li>
    </template>
